...
class VagrantConnector extends SSHConnector {
    ...
    async vagrantIsInstalled() {
        return new Promise(((resolve, reject) => {
            child_process.exec('vagrant --version', async (error, stdout, stderr) => {
                if (error || stderr) {
    ...
    async getContainerIp(context) {
        ...
        let count = (data.match(/ip/g) || []).length;
        // no ip address in file OR error getting data
        if (count === 0 || !data) return undefined;
    ...
    async getSshConfig() {
        const self = this;
        const VagrantfilePath = this.inCWD ? process.cwd() : (await this.getStatus()).directory;
        return new Promise(((resolve, reject) => {
            child_process.exec(`cd ${VagrantfilePath} && vagrant ssh-config`, (error, stdout, stderr) => {
                if (error || stderr) {
                    // console.error(`=> ${error}, ${stderr}`);
